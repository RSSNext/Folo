...
CREATE TABLE `__new_ai_chat_messages` (
	...
	`finished_at` integer,
	`message_parts` text,
	FOREIGN KEY (`chat_id`) REFERENCES `ai_chat_sessions`(`chat_id`) ON UPDATE no action ON DELETE cascade
);
...
CREATE TABLE `__new_ai_chat_sessions` (
...
--> statement-breakpoint
INSERT INTO `__new_ai_chat_sessions`("chat_id", "title", "created_at", "updated_at") SELECT "chat_id", "title", "created_at", "updated_at" FROM `ai_chat_sessions`;--> statement-breakpoint
DROP TABLE `ai_chat_sessions`;--> statement-breakpoint
ALTER TABLE `__new_ai_chat_sessions` RENAME TO `ai_chat_sessions`;--> statement-breakpoint
